...
class InterfazBiseccion:
    ...
    def crear_botones_funciones(self, parent):
        ...
        # Primera fila de botones: Trigonométricas básicas y recíprocas
        botones_funciones_1 = [
            ('sin x', 'sin(x)'), ('cos x', 'cos(x)'), ('tan x', 'tan(x)'),
            ('csc x', 'csc(x)'), ('sec x', 'sec(x)'), ('cot x', 'cot(x)')
        ]

        # Segunda fila de botones: Inversas e Hiperbólicas
        botones_funciones_2 = [
            ('arcsin x', 'asin(x)'), ('arccos x', 'acos(x)'), ('arctan x', 'atan(x)'),
            ('sinh x', 'sinh(x)'), ('cosh x', 'cosh(x)'), ('tanh x', 'tanh(x)')
        ]

        # Tercera fila de botones: Exponenciales y Logaritmos
        botones_funciones_3 = [
            ('e^x', 'exp(x)'), ('ln x', 'ln(x)'), ('log₁₀ x', 'log10(x)'),
            ('log₂ x', 'log2(x)'), ('logₓ x', 'logb(x,10)'), ('|x|', 'abs(x)')
        ]
        
        # Cuarta fila de botones: Raíces y Potencias
        botones_funciones_4 = [
            ('√x', 'sqrt(x)'), ('∛x', 'cbrt(x)'), ('ⁿ√x', 'root(x,3)'),
            ('x²', 'x^2'), ('x³', 'x^3'), ('xⁿ', 'x^')
        ]
        
        # Quinta fila de botones: Constantes y otros
        botones_funciones_5 = [
            ('π', 'pi'), ('e', 'e'), ('1/x', '1/x')
        ]
        ...
        for idx, (texto, valor) in enumerate(botones_funciones_5):
            btn = tk.Button(func_frame, text=texto, width=8, 
                           command=lambda v=valor: self.insertar_funcion(v))
            btn.grid(row=4, column=idx, padx=1, pady=1)
            ToolTip(btn, tooltips_funciones.get(valor, f'Insertar {valor}'))
    ...
    def crear_controles_entrada(self, parent):
        ...
        ToolTip(self.run_button, "Ejecuta el método de bisección para encontrar la raíz")
    ...
    def ejecutar_metodo(self):
        try:
            ...
            # Ejecutar método
            exito, resultado, iteraciones_data = ejecutar_metodo_biseccion(func_str, a, b, tolerance, max_iter)
