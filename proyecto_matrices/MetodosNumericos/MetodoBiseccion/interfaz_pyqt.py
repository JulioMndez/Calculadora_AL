#!/usr/bin/env python3
"""
Interfaz PyQt5 para el Método de Bisección
...
from metodo_biseccion import ejecutar_metodo_biseccion
...
class IterationsTableDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Tabla de Iteraciones - Método de Bisección")
        self.setModal(True)
        self.resize(800, 400)
...
class FunctionButtonsWidget(QWidget):
    ...
    def init_ui(self):
        ...
        functions = [
            ...
            [('exp', 'exp(x)'), ('ln', 'ln(x)'), ('log₁₀', 'log10(x)'), ('log₂', 'log2(x)'), ('logₓ', 'logb(x,10)'), ('|x|', 'abs(x)')],
            [('√', 'sqrt(x)'), ('³√', 'cbrt(x)'), ('ⁿ√', 'root(x,3)'), ('x²', 'x^2'), ('x³', 'x^3'), ('xⁿ', 'x^')],
            [('π', 'pi'), ('e', 'e'), ('1/x', '1/x')]
        ]
        ...
        tooltips = {
            ...
            'csc(x)': 'Digitar: csc(x)', 'sec(x)': 'Digitar: sec(x)', 'cot(x)': 'Digitar: cot(x)',
...
class InterfazBiseccionPyQt(QMainWindow):
    ...
    def create_steps_panel(self):
        ...
        # Título del panel
        title_label = QLabel("Método de Bisección - Paso a Paso")
    ...
    def solve_equation(self):
        """Resuelve la ecuación usando el método de bisección"""
        ...
        # Mostrar diálogo de espera
        progress = QProgressDialog("Calculando raíz por método de bisección...", "Cancelar", 0, 0, self)
        progress.setWindowTitle("Espere...")
        progress.setWindowModality(Qt.WindowModal)
        ...
        try:
            ...
            # Ejecutar método
            success, result, iterations = ejecutar_metodo_biseccion(func_str, a, b, tolerance, max_iter)
    ...
    def display_detailed_steps(self, iterations, result):
        ...
        steps_text = f"""MÉTODO DE BISECCIÓN
{'='*50}
...
Tolerancia: {self.tolerance_input.text()}"""
    ...
    def convert_to_latex(self, func_str):
        ...
        try:
            ...
            # --- Procesar Fracciones (Mejorado) ---
            def find_term_start(text, end_idx):
                i = end_idx - 1
                depth_paren = 0
                depth_brace = 0
            ...
            def find_term_end(text, start_idx):
                i = start_idx
                depth_paren = 0
                depth_brace = 0
                length = len(text)
            ...
            # Procesar fracciones de izquierda a derecha
            while '/' in latex:
                slash_idx = latex.find('/')
                
                # Encontrar numerador (hacia atrás)
                num_start = find_term_start(latex, slash_idx)
                numerator = latex[num_start:slash_idx].strip()
                
                # Encontrar denominador (hacia adelante)
                den_end = find_term_end(latex, slash_idx + 1)
                denominator = latex[slash_idx+1:den_end].strip()
            ...
            latex = re.sub(r'sec\(', r'\\sec\\left(', latex)
            latex = re.sub(r'csc\(', r'\\csc\\left(', latex)
            latex = re.sub(r'cot\(', r'\\cot\\left(', latex)
