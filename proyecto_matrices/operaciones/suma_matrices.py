...
def sumar_matrices(matrices, escalares, text_widget):
    ...
    for i in range(filas):
        for j in range(columnas):
            ...
            for m in range(len(matrices)):
                ...
                text_widget.insert(
                    tk.END,
                    f"  + ({escalares[m]}) * M{m+1}[{i},{j}] = {valor} → suma acumulada = {suma}\n"
                )
            resultado[i][j] = suma
    return resultado
...
def main():
    ventana = tk.Toplevel()
    ventana.title("Suma de Múltiples Matrices con Escalares")
    ...
    num_matrices = leer_entero_positivo("¿Cuántas matrices desea sumar? (mínimo 2)", 2, parent=ventana)
    if not num_matrices:
        ventana.destroy()
        return
    ...
    for m in range(num_matrices):
        ...
        # Pedir escalar para esta matriz
    ...
    resultado = sumar_matrices(matrices, escalares, text_box)
