...
import random
from proyecto_matrices.Run.styles import BG_COLOR, BUTTON_COLOR, ACCENT_COLOR, TEXT_COLOR, BUTTON_HOVER_COLOR, BUTTON_FONT, TITLE_FONT, MONO_FONT, style_tk_button, style_text
...
class AppEliminacionGaussiana:
    def __init__(self, root):
        ...
        tk.Label(setup_frame, text="Configuración del Sistema", font=self.title_font, bg="#3C3C3C", fg=TEXT_COLOR).pack(pady=(0, 15))
        dim_frame = tk.Frame(setup_frame, bg="#3C3C3C")
        dim_frame.pack(pady=5, padx=10)
        tk.Label(dim_frame, text="Ecuaciones:", font=self.default_font, bg="#3C3C3C", fg=TEXT_COLOR).grid(row=0, column=0, sticky="w")
        ...
        tk.Label(dim_frame, text="Incógnitas:", font=self.default_font, bg="#3C3C3C", fg=TEXT_COLOR).grid(row=1, column=0, sticky="w", pady=(5,0))
        ...
        btn_crear = tk.Button(actions_frame, text="Crear Matriz", command=self.crear_grid_matriz)
        style_tk_button(btn_crear)
        btn_crear.pack(side="left", padx=5)
        btn_ej = tk.Button(actions_frame, text="Ejemplo Aleatorio", command=self.generar_ejemplo_aleatorio)
        style_tk_button(btn_ej)
        btn_ej.pack(side="left", padx=5)
        btn_lim = tk.Button(actions_frame, text="Limpiar Matriz", command=self.limpiar_matriz)
        style_tk_button(btn_lim)
        btn_lim.pack(side="left", padx=5)
        self.matriz_frame = tk.Frame(setup_frame, bg="#3C3C3C")
        self.matriz_frame.pack(pady=10, padx=10, expand=True, fill="both")
        btn_res = tk.Button(setup_frame, text="Resolver Sistema", command=self.resolver, height=2)
        style_tk_button(btn_res)
        btn_res.pack(side="bottom", pady=10, fill="x")
        results_frame = tk.Frame(main_frame, bg="#3C3C3C")
        results_frame.pack(side="right", fill="both", expand=True)
        tk.Label(results_frame, text="Procedimiento Paso a Paso", font=self.title_font, bg="#3C3C3C", fg=TEXT_COLOR).pack(pady=10)
        self.pasos_text = scrolledtext.ScrolledText(results_frame, wrap=tk.WORD, height=20, font=self.mono_font, bg="#1E1E1E", fg=TEXT_COLOR, relief="sunken", borderwidth=1, insertbackground=TEXT_COLOR)
        self.pasos_text.pack(pady=5, padx=10, expand=True, fill="both")
        tk.Label(results_frame, text="Solución Final", font=self.title_font, bg="#3C3C3C", fg=TEXT_COLOR).pack(pady=(15, 5))
        self.solucion_widget = scrolledtext.ScrolledText(results_frame, wrap=tk.WORD, height=8, font=self.default_font, bg="#1E1E1E", fg=ACCENT_COLOR, relief="sunken", borderwidth=1)
