...
def mostrar_matriz(matriz):
    """Devuelve un string con la matriz en forma de lista de filas."""
    return "\n".join(["\t".join(f"{elem:.3g}" for elem in fila) for fila in matriz])

def gauss_eliminacion(matriz):
    """Reduce la matriz a forma escalonada y retorna el número de filas no nulas (rango)."""
    filas = len(matriz)
...
def main():
    ...
    # Determinar rango
    rango = gauss_eliminacion([fila[:] for fila in matriz])  # Copiar matriz para no modificar original
